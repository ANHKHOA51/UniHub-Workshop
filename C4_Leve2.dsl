workspace "UniHub Workshop" "Hệ thống quản lý đăng ký và check-in sự kiện" {
    model {
        ...
        unihub = softwareSystem "UniHub Workshop" "Hệ thống quản lý đăng ký và check-in sự kiện." {
            ...
            worker = container "Workers" "Tiến trình xử lý tác vụ nền: AI, Email, Import CSV." "Node.js, BullMQ"
            db = container "Primary Database" "Lưu trữ dữ liệu User, Workshop, Registration, Payment." "PostgreSQL" "Database"
            redis = container "Cache & Broker" "Lưu trữ Cache, Idempotency Key và Hàng đợi thông điệp." "Redis" "Database"
        }
        ...
        api_server -> db "Đọc/Ghi" "SQL/TCP"
        api_server -> redis "Đọc/Ghi & Phát Event" "RESP/TCP"
        worker -> db "Đọc/Ghi" "SQL/TCP"
        worker -> redis "Lắng nghe/Xử lý Event" "RESP/TCP"

        api_server -> momo "Yêu cầu thanh toán" "HTTPS/JSON"
